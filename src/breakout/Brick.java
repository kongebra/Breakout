...
public class Brick extends Rectangle {
	...
	public boolean collision(Ball ball) {
		// Most of the time, ball is far away from brick. Perform a fast integer
		// detection without extra addition.
		// All bricks are equal. Square vs square
		int relativeX = (int) ball.getCenterX() - (int) this.getX();
		if (relativeX < 0) {
			if (relativeX < fastNegativeWidth)
				return false; // Ball center is at least one radius left from
								// brick
		} else {
			if (relativeX > fastDoubleWidth)
				return false; // Ball center is at least one radius right from
								// brick
		}

		int relativeY = (int) ball.getCenterY() - (int) this.getY();
		if (relativeY < 0) {
			if (relativeY < fastNegativeHeight)
				return false; // Ball center is at least one radius above brick
		} else {
			if (relativeY > fastDoubleHeight)
				return false; // Ball center is at least one radius below brick
		}
		...
		if (relativeY >= 0 && relativeY <= rectHeight) {
			// If mostly Y movement, treat as corner if last X position would be
			// inside
			int relativeLastX = ball.getLastX() - (int) this.getX();
			if (!ball.contains(closestX, closestY) || // Ensure corner is inside
					relativeLastX < -ballR || relativeLastX > rectWidth + ballR) {
				double sign = 1;
		...
		if (relativeX >= 0 && relativeX <= rectWidth) {
			// If mostly X movement, treat as corner if last Y position would be
			// inside
			int relativeLastY = ball.getLastY() - (int) this.getY();
			if (!ball.contains(closestX, closestY) || // Ensure corner is inside
					relativeLastY < -ballR || relativeLastY > rectHeight + ballR) {
				double sign = 1;
				if (relativeY < rectHeight / 2)
					sign = -1;
